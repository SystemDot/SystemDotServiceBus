...
namespace SystemDot.Messaging.Specifications.messages.repeating.durable
{
    ...
    public class when_two_seconds_have_passed_after_a_stored_message_is_repeated_after_its_second_attempt
        : WithSubject<DurableMessageRepeater>
    {
        static MessagePayload processedMessage;
        static MessagePayload message;
        ...
        Establish context = () =>
        {
            ...
            DateTime currentDate = DateTime.Now;
            var currentDateProvider = new TestCurrentDateProvider(currentDate);
            Configure<ICurrentDateProvider>(currentDateProvider);
            
            Configure<ITaskScheduler>(new TestTaskScheduler(2, currentDateProvider));

            Subject.MessageProcessed += m => processedMessage = m;

            message = new MessagePayload();
            message.SetFromAddress(endpointAddress);
            message.SetLastTimeSent(currentDate);
            message.IncreaseAmountSent();
            message.IncreaseAmountSent();
            The<IMessageCache>().Cache(message);
        };
